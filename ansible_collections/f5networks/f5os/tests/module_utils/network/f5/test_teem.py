...
from ansible_collections.f5networks.f5os.plugins.module_utils.teem import (
    TeemClient, in_cicd, in_docker, generate_asset_id, determine_environment
)
from ansible_collections.f5networks.f5os.plugins.module_utils.version import CURRENT_COLL_VERSION
...
class TestTeemClient(TestCase):
    ...
    def test_teem_client_build_telemetry_no_docker_no_cicd(self, m, mock_plat):
        m.return_value = (False, None)
        mock_plat.return_value = 'rSeries'
    ...
    @patch.object(F5Client, 'platform', new_callable=PropertyMock)
    @patch('ansible_collections.f5networks.f5os.plugins.module_utils.teem.socket.gethostname', new_callable=Mock())
    @patch('ansible_collections.f5networks.f5os.plugins.module_utils.teem.in_cicd', new_callable=Mock())
    def test_teem_client_build_telemetry_with_docker_in_cicd(self, m1, m2, mock_plat):
    ...
    @patch.object(F5Client, 'platform', new_callable=PropertyMock)
    @patch('ansible_collections.f5networks.f5os.plugins.module_utils.teem.in_cicd', new_callable=Mock())
    def test_teem_client_build_telemetry_fq_name(self, m, mock_plat):
        m.return_value = (False, None)
    ...
    def test_teem_client_prepare_request(self):
        ...
        assert headers['User-Agent'] == 'F5OS/{0}'.format(CURRENT_COLL_VERSION)
    ...
    @patch('ansible_collections.f5networks.f5os.plugins.module_utils.teem.open_url')
    def test_teem_client_send(self, patched):
        ...
        assert patched.call_args[1]['headers']['User-Agent'] == 'F5OS/{0}'.format(CURRENT_COLL_VERSION)
...
class TestOtherFunctions(TestCase):
    def test_determine_environment_drone(self):
        ...
        with patch('ansible_collections.f5networks.f5os.plugins.module_utils.teem.os.getenv') as env:
