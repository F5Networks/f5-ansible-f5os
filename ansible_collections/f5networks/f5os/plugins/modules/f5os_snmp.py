...
class Difference(object):  # pragma: no cover
    ...
    def snmp_community(self):
        ...
        if getattr(self.want, 'snmp_community') != None:
    ...
    @property
    def snmp_target(self):
        '''Discrepancy between Module name (security_model,ipv4_address,ipv6_address,port) and API name (security-model,ipv4:{address,port},ipv6:{address,port})'''
        if getattr(self.want, 'snmp_target') != None:
            for wtarget in self.want.snmp_target:
                for htarget in self.have.snmp_target:
                    if wtarget['name'] == htarget['name']:
                        if 'security_model' in wtarget and wtarget['security_model'] != None:
                            if 'security-model' not in htarget:
                                return self.want.snmp_target
                            if wtarget['security_model'] != htarget['security-model']:
                                return self.want.snmp_target
                        
                        if 'community' in wtarget and wtarget['community'] != None:
                            if 'community' not in htarget:
                                return self.want.snmp_target
                            if wtarget['community'] != htarget['community']:
                                return self.want.snmp_target
                            
                        if 'user' in wtarget and wtarget['user'] != None:
                            if 'user' not in htarget:
                                return self.want.snmp_target
                            if wtarget['user'] != htarget['user']:
                                return self.want.snmp_target
                            
                        if 'ipv4_address' in wtarget and wtarget['ipv4_address'] != None:
                            ip_ver = 'ipv4'
                        ...
                        if 'ipv6_address' in wtarget and wtarget['ipv6_address'] != None:
                            ip_ver = 'ipv6'
                            if ip_ver not in htarget or 'address' not in htarget[ip_ver]:
                                return self.want.snmp_target
                        ...
                        if 'port' in wtarget and wtarget['port'] != None:
    ...
    def snmp_user(self):
        ...
        if getattr(self.want, 'snmp_user') != None:
    ...
    def snmp_mib(self):
        ...
        if getattr(self.want, 'snmp_mib') != None:
...
class ModuleManager(object):
    ...
    def exists(self)->bool:
        ...
        for object in ['snmp_community', 'snmp_target', 'snmp_user']:
            if (hasattr(self.want,object) and getattr(self.want,object) != None):
